...
export class Server {
	...
	setupEventHandlers() {
		...
		this.#connection.onDidChangeWatchedFiles(async () => {
			// Debounce config file changes to avoid excessive revalidation during multi-file updates
		...
		this.#connection.onDidSaveTextDocument((params) => {
			...
			const document = this.#documents.get(params.textDocument.uri);
			if (document) {
				// Clear any pending throttled validation to avoid duplicate work
		...
		this.#connection.onDidCloseTextDocument((params) => {
			...
			// Clean up config cache entries for this document to prevent memory leak
	...
	#getWorkspaceRootFor(documentUri) {
		...
		for (const folder of this.#workspaceFolders) {
			if (folder.uri?.startsWith("file:")) {
				...
				// Use path.relative for robust cross-platform subdirectory checking
