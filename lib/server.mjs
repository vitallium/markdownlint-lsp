...
export class Server {
	...
	setupEventHandlers() {
		...
		this.#connection.onDidCloseTextDocument((params) => {
			...
			this.#documentIssues.delete(params.textDocument.uri);
		...
		this.#connection.onCodeAction((params) => {
			...
			if (shouldProvideFixAll) {
				const issues = this.#documentIssues.get(uri) ?? [];
	...
	async validateDocument(document) {
		...
		const cacheKey = getConfigCacheKey(document.uri, workspaceRoot);
		...
		if (!documentOptions) {
			documentOptions =
				(await loadConfig(
					...
					{
						allowJavaScriptConfig: this.#allowJavaScriptConfig,
					},
				)) || {};
		...
		if (document.uri.startsWith("file:")) {
			...
			if (Object.hasOwn(mergedOptions, "ignores")) {
				...
				isIgnored = settingsIgnores.some((pattern) =>
				...
				isIgnored = mergedOptions._ignoreEntries.some(({ dir, patterns }) => {
			...
			if (isIgnored) {
				this.#logTrace(`Ignoring ${document.uri} (matched ignores pattern)`);
				this.#connection.sendDiagnostics({
					uri: document.uri,
					diagnostics: [],
				});
		...
		try {
			...
			this.#documentIssues.set(document.uri, issues);
			...
			this.#documentIssues.delete(document.uri);
			this.#connection.sendDiagnostics({
				uri: document.uri,
				diagnostics: [],
				version: currentVersion,
