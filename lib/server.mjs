			...
			if (shouldProvideQuickFix) {
				const requestedDiagnostics =
					params.context.diagnostics?.length > 0
						...
						: diagnosticFixPairs
								.filter((pair) =>
									this.#rangesOverlap(pair.diagnostic.range, params.range),
								)
								.map((pair) => pair.diagnostic);

				for (const requestedDiagnostic of requestedDiagnostics) {
