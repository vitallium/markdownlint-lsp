	...
	setupEventHandlers() {
		this.#connection.onInitialize(async (params) => {
			...
			if (params.rootPath) {
				...
				const primaryFolder = this.#workspaceFolders.find((folder) =>
					folder.uri?.startsWith("file:"),
				);
				if (primaryFolder?.uri) {
					this.#rootPath = fileURLToPath(primaryFolder.uri);
				}
			}
		...
		this.#connection.workspace.onDidChangeWorkspaceFolders((event) => {
			...
			if (
				...
				event.removed.some((folder) => {
					...
					const folderPath = fileURLToPath(folder.uri);
					return folderPath === this.#rootPath;
				})
			) {
				const nextFolder = this.#workspaceFolders.find((folder) =>
					folder.uri?.startsWith("file:"),
				);
				this.#rootPath = nextFolder?.uri ? fileURLToPath(nextFolder.uri) : null;
			}
