import path from "node:path";
import { fileURLToPath, pathToFileURL } from "node:url";
import { load } from "js-yaml";
import { parse as parseJsonc, printParseErrorCode } from "jsonc-parser";
import { readConfig as readMarkdownlintConfig } from "markdownlint/promise";
import {
	loadMarkdownlintIgnoreEntries,
	MARKDOWNLINT_IGNORE_FILENAME,
} from "./markdownlint-ignore.mjs";
import mergeOptions from "./merge-options.mjs";

// The order of these filenames is important and reflects the precedence
// that markdownlint-cli2 uses for configuration files.
// See: https://github.com/DavidAnson/markdownlint-cli2#configuration
export const MARKDOWNLINT_CLI2_CONFIG_FILENAMES = [
	".markdownlint-cli2.jsonc",
	".markdownlint-cli2.yaml",
	".markdownlint-cli2.yml",
	".markdownlint-cli2.cjs",
	".markdownlint-cli2.mjs",
];

const MARKDOWNLINT_CONFIG_FILENAMES = [
	".markdownlint.jsonc",
	".markdownlint.json",
	".markdownlint.yaml",
	".markdownlint.yml",
	".markdownlint.cjs",
	".markdownlint.mjs",
];

// RC-style configuration files (following https://www.npmjs.com/package/rc)
const MARKDOWNLINT_RC_CONFIG_FILENAMES = [
	".markdownlintrc",
	".markdownlint/config",
];

export const ALL_CONFIG_FILENAMES_EXCEPT_PACKAGE_JSON = [
	...MARKDOWNLINT_CLI2_CONFIG_FILENAMES,
	...MARKDOWNLINT_CONFIG_FILENAMES,
	...MARKDOWNLINT_RC_CONFIG_FILENAMES,
];

export { MARKDOWNLINT_IGNORE_FILENAME };

const PARSERS = [(content) => load(content)];
// JSONC parser wrapper follows markdownlint-cli2's established implementation.
const JSONC_PARSERS = [
	(content) => {
		const errors = [];
		const result = parseJsonc(content, errors, { allowTrailingComma: true });
		if (errors.length > 0) {
			const details = errors
				.map(
					(error) =>
						`${printParseErrorCode(error.error)} (offset ${error.offset}, length ${error.length})`,
				)
				.join(", ");
			throw new Error(`Unable to parse JSONC content, ${details}`);
		}
		return result;
	},
	...PARSERS,
];
const MAX_LOG_CONFIG_LENGTH = 2000;
const SENSITIVE_KEY_PATTERN = /token|secret|password|api[_-]?key|auth/i;

function redactConfig(value) {
	if (Array.isArray(value)) {
		return value.map((item) => redactConfig(item));
	}
	if (value && typeof value === "object") {
		if (typeof value.ignores === "function") {
			return "<<ignore instance>>";
		}
		const redacted = {};
		for (const [key, entry] of Object.entries(value)) {
			if (SENSITIVE_KEY_PATTERN.test(key)) {
				redacted[key] = "<<redacted>>";
				continue;
			}
			redacted[key] = redactConfig(entry);
		}
		return redacted;
	}
	return value;
}

function formatConfigForLog(value) {
	try {
		const sanitized = redactConfig(value);
		const serialized = JSON.stringify(sanitized, null, 2);
		if (serialized.length <= MAX_LOG_CONFIG_LENGTH) {
			return serialized;
		}
		return `${serialized.slice(0, MAX_LOG_CONFIG_LENGTH)}... (truncated)`;
	} catch (error) {
		return `<<unserializable config: ${error}>>`;
	}
}

async function loadJsConfig(configPath) {
	const url = pathToFileURL(configPath);
	url.search = `?t=${Date.now()}`;
	const moduleExports = await import(url.href);
	return moduleExports.default ?? moduleExports;
}

function normalizeLoadConfigOptions(loggerOrOptions, maybeOptions) {
	if (typeof loggerOrOptions === "function") {
		return {
			logger: loggerOrOptions,
			allowJavaScriptConfig: maybeOptions?.allowJavaScriptConfig ?? false,
		};
	}

	const options = loggerOrOptions ?? {};
	return {
		logger: options.logger ?? (() => {}),
		allowJavaScriptConfig: options.allowJavaScriptConfig ?? false,
	};
}

function isSubdirectory(parent, child) {
	if (child === parent) return true;
	const relative = path.relative(parent, child);
	return relative && !relative.startsWith("..") && !path.isAbsolute(relative);
}

async function findConfigFileInDir(
	dir,
	filenames,
	logger,
	allowJavaScriptConfig,
) {
	for (const filename of filenames) {
		const configPath = path.join(dir, filename);
		try {
			const isJsConfig =
				configPath.endsWith(".cjs") || configPath.endsWith(".mjs");
			if (isJsConfig && !allowJavaScriptConfig) {
				logger(`Skipping JavaScript config file: ${configPath}`, true);
				continue;
			}
			const config = isJsConfig
				? await loadJsConfig(configPath)
				: await readMarkdownlintConfig(
						configPath,
						configPath.endsWith(".jsonc") ? JSONC_PARSERS : PARSERS,
					);

			if (filename === "package.json") {
				if (config["markdownlint-cli2"]) {
					logger(`Found config in ${configPath}`, true);
					return {
						filepath: configPath,
						config: config["markdownlint-cli2"],
					};
				}
				// If no `markdownlint-cli2` key, it's not a config file for us.
				continue;
			}

			if (config) {
				logger(`Found config file: ${configPath}`, true);
				return { filepath: configPath, config };
			}
		} catch (error) {
			if (error.code !== "ENOENT" && error.code !== "EISDIR") {
				logger(`failed to load or parse ${configPath}: ${error}`, "warning");
			}
		}
	}
	return null;
}

/**
 * Loads configuration for a given file, traversing up the directory tree
 * from the file's location to the workspace root, and merging all found
 * configurations.
 *
 * @param {string} fileUri The URI of the markdown file.
 * @param {string} workspaceRoot The absolute path to the workspace root.
 * @param {Function} [logger=()=>{}] Optional logger function.
 * @returns {Promise<object|null>} A promise that resolves to the final
 * markdownlint options object, or null if no configuration is found.
 */
export async function loadConfig(
	fileUri,
	workspaceRoot,
	loggerOrOptions = () => {},
	maybeOptions = {},
) {
	const { logger, allowJavaScriptConfig } = normalizeLoadConfigOptions(
		loggerOrOptions,
		maybeOptions,
	);

	if (!fileUri.startsWith("file:")) {
		logger("Skipping config load for non-file URI", true);
		return null;
	}

	const filePath = fileURLToPath(fileUri);
	const startDir = path.dirname(filePath);

	const directoriesToSearch = [];
	let currentDir = startDir;

	while (isSubdirectory(workspaceRoot, currentDir)) {
		directoriesToSearch.push(currentDir);
		if (currentDir === workspaceRoot) {
			break;
		}
		const parentDir = path.dirname(currentDir);
		if (parentDir === currentDir) {
			break;
		}
		currentDir = parentDir;
	}

	const configPromises = directoriesToSearch.map(async (dir) => {
		let cli2Config = await findConfigFileInDir(
			dir,
			MARKDOWNLINT_CLI2_CONFIG_FILENAMES,
			logger,
			allowJavaScriptConfig,
		);

		const mlConfig = await findConfigFileInDir(
			dir,
			[...MARKDOWNLINT_CONFIG_FILENAMES, ...MARKDOWNLINT_RC_CONFIG_FILENAMES],
			logger,
			allowJavaScriptConfig,
		);

		if (dir === workspaceRoot && !cli2Config) {
			cli2Config = await findConfigFileInDir(
				dir,
				["package.json"],
				logger,
				allowJavaScriptConfig,
			);
		}

		if (cli2Config && mlConfig) {
			logger(
				`Found both CLI2 and markdownlint configs in ${dir}, markdownlint config overrides CLI2's config property`,
				true,
			);
			return {
				filepath: cli2Config.filepath,
				filepaths: [cli2Config.filepath, mlConfig.filepath],
				config: { ...cli2Config.config, config: mlConfig.config },
			};
		}

		return cli2Config || mlConfig || null;
	});

	const foundConfigs = (await Promise.all(configPromises)).filter(Boolean);
	const markdownlintIgnoreEntries = await loadMarkdownlintIgnoreEntries(
		directoriesToSearch,
		logger,
	);

	if (foundConfigs.length === 0) {
		logger(
			`no config found between ${startDir} and workspace root; using defaults`,
			"info",
		);
	}

	if (foundConfigs.length === 0 && markdownlintIgnoreEntries.length === 0) {
		logger("No config files found in the file's path.", true);
		return null;
	}

	let mergedOptions = {};
	let ignoreEntries = [];
	for (const { config, filepath, filepaths } of foundConfigs.reverse()) {
		const filename = path.basename(filepath);
		logger(`Applying config from ${filepath}`, true);
		for (const configFilepath of filepaths ?? [filepath]) {
			logger(`using config ${configFilepath}`, "info");
		}

		// Check if this is a markdownlint-specific config file (not CLI2)
		const isMarkdownlintConfig =
			MARKDOWNLINT_CONFIG_FILENAMES.includes(filename) ||
			filepath.endsWith(".markdownlintrc") ||
			filepath.endsWith(".markdownlint/config");

		if (isMarkdownlintConfig) {
			mergedOptions.config = config;
		} else {
			// Track the effective ignores from CLI2 configs while preserving merge
			// semantics (closer config overrides parent when `ignores` is present).
			if (Object.hasOwn(config, "ignores")) {
				if (Array.isArray(config.ignores) && config.ignores.length > 0) {
					ignoreEntries = [
						{
							dir: path.dirname(filepath),
							patterns: config.ignores,
						},
					];
				} else {
					ignoreEntries = [];
				}
			}
			mergedOptions = mergeOptions(mergedOptions, config);
		}
	}

	if (Object.hasOwn(mergedOptions, "ignores")) {
		delete mergedOptions.ignores;
	}
	if (ignoreEntries.length > 0) {
		mergedOptions._ignoreEntries = ignoreEntries;
	}
	if (markdownlintIgnoreEntries.length > 0) {
		mergedOptions._markdownlintIgnoreEntries = markdownlintIgnoreEntries;
	}
	if (Object.keys(mergedOptions).length === 0) {
		return null;
	}
	if (logger) {
		logger(`Final merged config: ${formatConfigForLog(mergedOptions)}`, true);
	}
	return mergedOptions;
}
