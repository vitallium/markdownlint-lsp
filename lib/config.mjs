...
const ALL_CONFIG_FILENAMES_EXCEPT_PACKAGE_JSON = [
...
export async function loadConfig(fileUri, workspaceRoot, logger = () => {}) {
	...
	for (const { config, filepath } of foundConfigs.reverse()) {
		...
		if (MARKDOWNLINT_CONFIG_FILENAMES.includes(filename)) {
	...
	if (Object.keys(mergedOptions).length > 0) {
		if (logger) {
			logger(
				`Final merged config: ${JSON.stringify(mergedOptions, null, 2)}`,
				true,
			);
		}
