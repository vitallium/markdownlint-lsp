...
export async function readConfig(dir, logger = null) {
    ...
    if (matchedFile) {
      try {
        if (filename === "package.json") {
          ...
          if (config) {
            if (logger) {
              logger("Read config from package.json", true);
            }
            return config;
          }
        ...
        if (config) {
          if (logger) {
            logger(`Read config from ${filename}`, true);
          }
          return config;
        }
      } catch (error) {
        ...
        if (logger) {
          logger(`Error reading or parsing ${matchedFile}: ${error}`, true);
        }
      }
    }
  }
